...
async def on_voice_state_update(member, before, after):
    ...
    # --- VC-1 の管理 ---
    # ホストが一人目として入室した場合（配信開始）
    if before.channel != vc1 and after.channel == vc1 and hosts_in_vc1 == 1:
        print(f"> VC-1 配信開始 ({member.display_name})：接続を許可します。")
        
        # 【修正】接続(connect)だけでなく、チャット(send_messages)と閲覧(view_channel)も明示的に許可します
        await vc1.set_permissions(viewer_role, connect=True, view_channel=True, send_messages=True)
    ...
    # ホストが最後の一人として退室した場合（配信終了）
    elif before.channel == vc1 and after.channel != vc1 and hosts_in_vc1 == 0:
        print(f"> VC-1 配信終了 ({member.display_name})：接続を禁止し、視聴者を退出させます。")
        
        # 【★ここが重要】
        # 接続(connect)は False ですが、チャット(send_messages)と閲覧(view_channel)は True (許可) のままにします
        await vc1.set_permissions(viewer_role, connect=False, view_channel=True, send_messages=True)
